...
def parce_request(message):
    ...
    if (message.message_thread_id == s.translation_chat_topic) or (message.message_thread_id == 956) or ("#translate" in message.text) or ("#t" in message.text):  # if topic == Translate to Eng
        ...
        try:
            ...
            lg.log_message("ERROR", "Translation. Can't translate the message from user " + str(message.from_user.id) + " (" + message.from_user.username+")"+". Message: " + message.text)
    ...
    # Find mistakes in the received message if posted in Spell checker topic
        ...
        print(message.text)
        try:
            is_errors, corrected_message = lang.check_mistakes(lang.remove_telegram_emojis(message.text),False,False)
            if not is_errors:
            ...
            lg.log_message("ERROR", "Checking. Can't check the message from user " + str(message.from_user.id) + " (" + message.from_user.username+")"+". Message: " + message.text)
        ...
        try:
            #raise TypeError("Input text must be a string.")  # this is for testing to raise exception
            is_errors, corrected_message = lang.check_mistakes(lang.remove_telegram_emojis(message.text),False,True)
            if not is_errors:
                send_ok_react(message.chat.id, message.message_id)
            else:
                bot.reply_to(message, lang.remove_tags(corrected_message), parse_mode='HTML')
            ...
            print(">>> An error during checking occurred:", e)
            lg.log_message("ERROR", "Checking. Can't do full check for the message from user " + str(message.from_user.id) + " (" + message.from_user.username+")"+". Message: " + message.text)
